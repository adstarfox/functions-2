...
const print = (para1, para2) => `The first parameter is ${para1}. the second parameter is ${para2}.`



// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.
console.log(print(`steve`,`John`))
...
const greeting = (firstName, lastName, cb) =>{
    return `Hello, my full name is ${cb(firstName,lastName)}`
    
}

console.log(greeting(`Alex`,`Durrant`,fullName))
...
const displayArr = (theTotal) => console.log(`The new array plus tax = ${theTotal}`)
...
pricesPlusTax(prices,displayArr)
...
const multiplyingFactory = x => y =>{
    if (x >= 5){
      ...
      console.log(`Cannot multipy: the first number is smaller than 5`)
    }
}
